/**
 * Tool: query_trade_monthly_by_countries
 *
 * 查詢按國家分組的月度貿易統計資料 (trade_monthly_by_countries)。
 *
 * 此表提供台灣按月度、國家與產業群組彙總的進出口貿易數據，
 * 包含地區歸屬資訊（AREA_ID, AREA_NM），可直接按洲別分析。
 */
console.log('loading query_trade_monthly_by_countries tool');
...
export const name = 'query_trade_monthly_by_countries';

export const description =
  `查詢按國家分組的月度貿易統計資料。

用途：提供台灣按月度、國家與產業群組彙總的進出口貿易數據。
包含地區歸屬資訊，可直接按洲別/經濟區域分析。
...
常見使用場景：
1. 查詢 2024 年對美國各月出口:
   year: 2024, tradeFlow: "出口", country: "US", order: "ASC"
2. 查詢東南亞各國月度進口:
   year: 2024, tradeFlow: "進口", country: "東南亞"
3. 查詢電子產業對日本月度出口:
   year: 2024, tradeFlow: "出口", country: "JP", industryKeyword: "電子"`;
...
export const execute = async (params) => {
  ...
  try {
    const RESOLVER = 'trade_monthly_by_country';
    ...
    return {
      content: [{
        type: 'text',
        text: JSON.stringify({ error: 'Trade monthly by countries query failed', details: err.message }),
      }],
      isError: true,
    };
  }
