...
export const execute = async (params) => {
  ...
  try {
    const { query } = buildQuery('trade_monthly_growth_by_country', normalizedParams);
    ...
    return {
      content: [{ type: 'text', text: JSON.stringify({ error: 'Trade monthly growth by countries query failed', details: err.message }) }],
      isError: true,
    };
  }
};
